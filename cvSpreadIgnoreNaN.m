function [ meanEuclidianDist, meanSquaredDist, CI, CIDistribution ] = cvSpreadIgnoreNaN( obs, classIdx, CIMode, CIAlpha, CIResamples )
    ...
    meanOfSquares = mean(squaredDistEstimates, 1); % mean over folds: nFolds x nClasses x nTime --> 1 x nClasses x nTime
    medianUnbiasedEstimate = sign(meanOfSquares).*sqrt(abs(meanOfSquares)); 

    meanSquaredDist = mean(meanOfSquares, 2, 'omitnan'); % 1 x nClasses x nTime --> 1 x 1 x nTime
    meanEuclidianDist = mean(medianUnbiasedEstimate, 2, 'omitnan');
...
function output = ciWrapper(varargin)
    ...
    [ medianUnbiasedEstimate, meanOfSquares ] = cvSpreadIgnoreNaN( allObs, allClassIdx );
